...
use reth::revm::{
    db::{BundleState, EmptyDB, TransitionState},
...
use super::{cache::state::ThreadSafeCacheState, state::SharableState};

/// Allows building of State and initializing it with different options.
#[derive(Clone, Debug)]
pub struct SharableStateBuilder<DB> {
    ...
    with_bundle_prestate: Option<Arc<RwLock<BundleState>>>,
    ...
    with_block_hashes: Arc<RwLock<BTreeMap<u64, B256>>>,
}

impl SharableStateBuilder<EmptyDB> {
...
impl<DB: Database + Default> Default for SharableStateBuilder<DB> {
    fn default() -> Self {
        Self::new_with_database(DB::default())
    }
}

impl<DB: Database> SharableStateBuilder<DB> {
    ...
    pub fn with_database_ref<ODB: DatabaseRef>(
        self,
        database: ODB,
    ) -> SharableStateBuilder<WrapDatabaseRef<ODB>> {
    ...
    ) -> SharableStateBuilder<DBBox<'_, Error>> {
    ...
    pub fn with_cached_prestate(self, cache: ThreadSafeCacheState) -> Self {
        Self {
    ...
    pub fn with_block_hashes(self, block_hashes: Arc<RwLock<BTreeMap<u64, B256>>>) -> Self {
        Self {
