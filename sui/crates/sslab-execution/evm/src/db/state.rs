...
use super::{
    ...
    cache::{cache_account::CacheAccount, state::ThreadSafeCacheState},
};
...
pub struct SharableState<DB> {
    ...
    pub cache: ThreadSafeCacheState,
    ...
    pub transition_state: Option<Arc<RwLock<TransitionState>>>,
    ...
    pub block_hashes: Arc<RwLock<BTreeMap<u64, B256>>>,
}

// Have ability to call State::builder without having to specify the type.
impl SharableState<EmptyDB> {
    /// Return the builder that build the State.
    pub fn builder() -> SharableStateBuilder<EmptyDB> {
        SharableStateBuilder::default()
    }
}

impl<DB: Database> SharableState<DB> {
    ...
    pub fn bundle_size_hint(&self) -> usize {
        self.bundle_state.read().size_hint()
    }
    ...
    pub fn increment_balances(
        ...
        if let Some(s) = self.transition_state.as_ref() {
            s.write().add_transitions(transitions)
        }
        Ok(())
    }
    ...
    /// is applied.
    pub fn merge_transitions(&self, retention: BundleRetention) {
        if let Some(transition_state) = self.transition_state.as_ref() {
            let transition_state = transition_state.write().take();
            self.bundle_state
                .write()
    ...
    pub fn load_cache_account_mut(
        &self,
        address: Address,
    ) -> Result<dashmap::mapref::one::RefMut<Address, CacheAccount>, DB::Error> {
        // dashmap::mapref::one::MappedRefMut::map(&self.cache, |cache|)
        Ok(match self.cache.accounts.entry(address) {
    ...
    pub fn load_cache_account(
        &self,
        address: Address,
    ) -> Result<Ref<Address, CacheAccount>, DB::Error> {
        // dashmap::mapref::one::MappedRefMut::map(&self.cache, |cache|)
        Ok(match self.cache.accounts.entry(address) {
            dashmap::mapref::entry::Entry::Vacant(entry) => {
                if self.use_preloaded_bundle {
                    ...
                    if let Some(account) = self
                        ...
                        .map(Into::<CacheAccount>::into)
                    {
                        return Ok(entry.insert(account.clone()).downgrade());
                    }
                }
                // if not found in bundle, load it from database
                let info = self.database.basic(address).ok().unwrap(); //TODO: process DB::Error
    ...
    pub fn take_bundle(&mut self) -> BundleState {
        core::mem::take(&mut self.bundle_state.write())
    }
}

impl<DB: Database> Database for SharableState<DB> {
    type Error = DB::Error;
    ...
    fn code_by_hash(&self, code_hash: B256) -> Result<Bytecode, Self::Error> {
        let res = match self.cache.contracts.entry(code_hash) {
            ...
            dashmap::mapref::entry::Entry::Vacant(entry) => {
                if self.use_preloaded_bundle {
                    if let Some(code) = self.bundle_state.read().contracts.get(&code_hash) {
    ...
    fn block_hash(&self, number: U256) -> Result<B256, Self::Error> {
        ...
        let tmp = match self.block_hashes.write().get(&u64num) {
        ...
        match tmp {
            ...
            None => {
                ...
                self.block_hashes.write().insert(u64num, ret);

                // prune all hashes that are older then BLOCK_HASH_HISTORY
                while let Some(entry) = self.block_hashes.write().first_entry() {
...
impl<DB: Database> DatabaseCommit for SharableState<DB> {
...
mod tests {
    ...
    use reth::revm::interpreter::primitives::{keccak256, StorageSlot};
    ...
    fn block_hash_cache() {
        ...
        cur.extend(state.block_hashes.read().iter());
