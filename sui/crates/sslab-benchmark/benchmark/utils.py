...
class PathMaker:
    ...
    @staticmethod
    def node_crate_path():
        return join('..', 'sslab-core', 'src')
    
    @staticmethod
    def client_crate_path():
        return join('..', 'sslab-benchmark')
...
def join_with_progress_bar(threads, prefix='', suffix='', decimals=1, length=30, fill='█', print_end='\r'):
    total = len(threads)
